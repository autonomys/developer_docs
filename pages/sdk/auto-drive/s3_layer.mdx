...
Auto Drive maintains an `object_mappings` table in the database that maps S3 (bucket, key) pairs to Content Identifiers (CIDs) and MD5 checksums. When you upload via the S3 API, the system:
...
`PutObject`, `GetObject`, `HeadObject`, `ListObjectsV2`, and `CompleteMultipartUpload` all return a **standard quoted MD5 ETag** (e.g. `"d41d8cd98f00b204e9800998ecf8427e"`). For multipart uploads, the composite ETag follows the AWS format `"<md5_of_part_md5s>-<N>"`.
...
| **Local development** | `http://localhost:3000/s3` |
...
const s3Client = new S3Client({
  ...
  forcePathStyle: true,                               // required: bucket lives in the path
});
```
...
- Files are stored on the Distributed Storage Network of the Autonomys Network (Mainnet & Testnet)
...
For the dedicated walkthrough — quickstart, full config reference, every supported command, workflows, and troubleshooting — see the [Using rclone](./rclone) guide.
...
- Listings paginate at the server's configured `maxKeys` (default 1,000)
