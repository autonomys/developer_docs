...
Both use the MD5 ETag for verification. Add `--ignore-checksum` for legacy objects without a stored MD5.
...
- `rclone delete` — returns `403 Access Denied: Auto Drive storage is immutable.`
...
```
ERROR : file.txt: Failed to delete: 403 Access Denied: Auto Drive storage is immutable.
```
...
- [Auto Drive S3 Layer Guide](./s3_layer) — the underlying S3-compatible API used by rclone
