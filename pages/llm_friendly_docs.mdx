...
### File: auto_drive/pay_with_ai3.mdx
...
> **AI3 on Auto EVM required.** Payments use AI3 on the Auto-EVM network, not on the consensus chain. If your AI3 is on the consensus chain, you will need to bridge it to Auto EVM first — see the [Bridging guide](https://docs.autonomys.xyz/bridge).

---

## Step 1: Open Purchase Credits

Sign in to [ai3.storage](https://ai3.storage/) and click **"Buy more credits"** in the bottom-left sidebar of the dashboard.
...
Choose one of the preset packages or configure a custom amount. The current price per MB is shown at the top of the page and reflects live protocol-level DSN storage costs and a service fee.
...
Your wallet must hold **AI3 on the Auto-EVM network** (Chain ID 870). If your AI3 is on the consensus chain, bridge it first — see the [Bridging guide](https://docs.autonomys.xyz/bridge). If needed, add Auto EVM to your wallet:
...
Your credits are immediately available and will be reflected in the sidebar balance. Credits are consumed in **FIFO expiry order** — credits closest to expiration are spent first. Your free monthly allocation is always spent before purchased credits.
...
### File: evm/farmerless_dev_node.mdx
...
For easy setup using Docker, you can use the provided \`docker-compose.yml\` file. This setup automatically builds and runs the farmerless dev node with the EVM domain enabled.
...
### File: sdk/auto-drive/encryption.mdx
...
### File: sdk/auto-drive/pay_with_ai3.mdx
...
2. The backend creates a **payment intent** — a server-side record that locks the price and binds a \`bytes32\` intent ID to the authenticated account for up to 10 minutes.
...
Credits are available immediately after confirmation and can be used via the dashboard, the AutoSDK, or the REST API.
...
> Always fetch a fresh quote immediately before creating an intent. Prices reflect live protocol costs and can change between requests.
...
> **Authentication:** Requires a Google OAuth token in the \`Authorization\` header. The intent is bound to this account — credits cannot be redirected elsewhere.
...
Credits are consumed in **FIFO expiry order** — the soonest-to-expire credits are spent first. Free monthly credits are consumed before purchased ones.
