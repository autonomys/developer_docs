# Account Balances and Transaction Fees on Taurus

## Introduction

While Taurus strives to maintain EVM compatibility, there are some important differences that developers should understand regarding account balances and transaction fees. These differences stem from Taurus's Substrate-based architecture and affect how balances are managed and fees are calculated.
...
In addition to standard gas fees, Taurus implements consensus storage fees:
...
When working with smart contracts on Taurus, be aware that:
...
1. Balance Management:
   ...
   - Taurus: Requires ED maintenance

2. Fee Structure:
   - Ethereum: Gas fees only
   - Taurus: Gas fees + consensus storage fees

3. Contract Behavior:
   - Ethereum: Contracts can be fully emptied
   - Taurus: ED must remain in contracts
