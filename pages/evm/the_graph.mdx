...
2. Then we'll change the `Counter` contract state and set the number to `5` by running `cast send YOUR_CONTRACT_ADDRESS "setNumber(uint256)" 5 --rpc-url https://auto-evm-0.chronos.autonomys.xyz/ws --private-key YOUR_KEY`

3. Let's trigger one more event in the `Counter` contract by calling the `increment()` function. We can do that by running `cast send YOUR_CONTRACT_ADDRESS "increment()" --rpc-url https://auto-evm-0.chronos.autonomys.xyz/ws --private-key YOUR_KEY`
