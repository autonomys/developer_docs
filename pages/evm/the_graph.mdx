...
1. Let's make some slight changes to the `Counter` contract we've introduced in the [Foundry guide](/evm/foundry.mdx) and modify functions to emit events. 
...
> Tip: you can also run queries directly from the terminal using `cURL` or `gql`. You can find the **Example Usage** under the **Endpoints** tab. 
